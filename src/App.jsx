...
export default function App() {
  ...
  useEffect(() => {
    const filterLabel = filter === "buy" ? "Buy Fooods"
    : filter === "not buy" ? "Food I dont buy"
    : "Food Recipes";
    document.title =`${filterLabel} (${visibleRecipes.length})`;
  }, [filter, visibleRecipes.length]);
  ...
  const handleToggleFavorite = (id) => {
    setRecipes(
      recipes.map((recipe) =>
        recipe.id ===id ? {...recipe, buy: !recipe.buy}: recipe
    ) 
    );
  };
  ...
  const handleVisibleRecipes = () => {
    if (confirm("Show All recipes? all.")){
      setRecipes([]);
    }
  };
